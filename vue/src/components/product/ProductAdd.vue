    ...
    <el-card class="product-box" shadow="hover">
      ...
      <div class="info-box">
        <el-form class="product-form" :model="productForm" :inline="true" :rules="productRules" ref="ruleForm">
          <el-form-item label="书籍名称：" prop="productName" style="width: 100%">
            <el-input size="small" v-model="productForm.productName" style="width: 300px"></el-input>
          </el-form-item>
          <el-form-item label="书籍分类：" prop="productType" style="width: 45%">
          ...
          <el-form-item label="书籍规格：" prop="productSpec" style="width: 50%">
            <el-select size="small" class="select-spec" v-model="productSpec" multiple placeholder="请选择书籍规格" style="width: 245px;">
              <el-option v-for="item in productSpecs"  :key="item.specsId" :label="item.specsName" :value="item.specsId"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="书籍品牌：" prop="productBrand" style="width: 100%">
          ...
          <el-form-item label="书籍进价：" prop="inPrice" style="width: 45%;">
            <el-input min=0 size="small" type='number' v-model="productForm.inPrice" placeholder="书籍进价"></el-input>
          </el-form-item>
          <el-form-item label="书籍库存：" prop="productStock" style="width: 45%;">
            <el-input-number size="small" :min=0 :max=1000 v-model="productForm.productStock" placeholder="书籍库存"></el-input-number>
          </el-form-item>
          <el-form-item label="书籍售价：" prop="outPrice" style="width: 45%;">
            <el-input min=0 size="small" type='number' v-model="productForm.outPrice" placeholder="书籍售价"></el-input>
          </el-form-item>
          ...
          <el-form-item label="书籍状态：" style="width: 100%;height: 30px">
          ...
          <el-form-item label="书籍介绍：" prop="productDescribe" style="width: 45%;">
            <el-input type="textarea" class="product-desc" size="small" show-word-limit :autosize="{ minRows: 1, maxRows: 4}"  v-model="productForm.productDescribe" placeholder="书籍描述"></el-input>
          </el-form-item>
          <el-form-item label="书籍图片：" prop="productUrl" style="width: 45%;">
            <el-upload
          ...
          <el-form-item style="width:100%;text-align: center;position: absolute;bottom: -14px;left: 0">
            <el-button  @click="submitForm('ruleForm')" type="primary" style="margin: 0 55px">添加书籍</el-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        productSpecs:[],
        productRules:{
          productName: [{ required: true, message: '请填写书籍名称', trigger: 'blur' },
                  { min: 1, max: 30, message: '名称长度不允许超过 30 个字符', trigger: 'blur' }],
          productBrand: [{ required: true, message: '请选择书籍品牌', trigger: 'blur' }],
          productType: [{ required: true, message: '请选择书籍分类', trigger: 'blur' }],
          productDescribe: [{ required: true, message: '请填写书籍介绍', trigger: 'blur' },
            { min: 1, max: 100, message: '名称长度不允许超过 100 个字符', trigger: 'blur' }],
          productUrl: [{ required: true, message: '请上传书籍图片', trigger: 'blur' }],
        }
      }
    },
    methods: {

      ...
      <div class="cart-info" v-if="tableData.length!==0">
        ...
        <div class="cart-bar">
          ...
          <div class="section-right">
            ...
            <el-tooltip content="请勾选需要结算的书籍" v-if="selectedData" :disabled="selectedData.length!==0" placement="top" effect="light">
              <div class="button">
    ...
    createOrder() {
      ...
      for (let index = 0, len = this.selectedData.length; index < len; index++) {
        let order = {
          ...
          payPrice: this.baseInfo.payMoney,
      ...
      this.$http.post('/order/cartOrder', this.$qs.stringify(send)).then((rep) => {
        ...
        if (rep.data.code === 200) {
          ...
          for (let i = 0; i < this.selectedData.length; i++) {
            let index = this.tableData.findIndex(item => item.cartId === this.selectedData[i].cartId);
            if (index !== -1) {
              this.tableData.splice(index, 1);
              // 在此处发送删除购物车商品的请求
              this.$http.post('/shoppingCart/deleteById?cartId='+this.selectedData[i].cartId).then((rep) => {
                if (rep.data.code === 200) {
                  this.reqInfo();
